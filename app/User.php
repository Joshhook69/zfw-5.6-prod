...
class User extends Eloquent implements UserInterface {
    ...
    protected $fillable = array('id', 'first_name', 'last_name', 'email', 'rating_id', 'canTrain', 'visitor', 'visitor_from', 'status', 'loa', 'del', 'gnd', 'twr', 'app', 'ctr', 'max');
    ...
    public function getStaffLongAttribute()
    {
        switch($this->staff_role)
        {
            ...
            case 'AFE': return "Assistant Facility Engineer";
    ...
    public function getTwrCertAttribute()
    {
        switch($this->twr)
        {
            case 0: return "No Certification";
            case 1: return "Minor Training";
            case 2: return "Solo Certified";
            case 3: return "Minor Certified";
            case 4: return "Major Training";
            case 5: return "Major Certified";
    ...
    {
        require_once("/home/fortworth/www/zfwartcc.net/public/forum/smf_2_api.php");
    ...
    public function sendVReqDeclineEmail() {
        return Mail::send('emails.visitdeny', ['session' => $this], function($message){
            ...
            $message->subject('vZFW - Visiting Application Denied');
        });
    }
    ...
    public function sendVReqUpdateEmail() {
        return Mail::send('emails.2weekvisitr', ['session' => $this], function($message){
            $message->to($this->visitr->email);
            $message->subject('vZFW - Visiting Application Update');
        });
    }

    public function hasStaff()
    {
        if($this->hasRole('ATM') || $this->hasRole('DATM') || $this->hasRole('TA') || $this->hasRole('WM') || $this->hasRole('ATA') || $this->hasRole('AEC') || $this->hasRole('EC') || $this->hasRole('AFE') || $this->hasRole('FE') || $this->hasRole('AWM')) {
            return true;
        } else {
            return false;
        }
