...
class FrontController extends BaseController {
    ...
    {
        $online = ATC::all();
        $weather = Weather::whereIn('id', ['KDFW','KDAL','KADS','KGPM','KOKC','KACT','KLBB'])->get();
    ...
    public function showDocuments()
    {
        ...
        return View::make('site.documents',[
            ...
            'reference'    => $Documents->filter($byType('reference')),
        ]);
    }
    ...
    {
        $kdfw = Weather::find('KDFW')->with('D10_flow', 'north');
        $kdal = Weather::find('KDAL')->with('D10_flow', 'north');
        $kokc = Weather::find('KOKC');

        return View::make('site.runways')->with('kdfw', $kdfw)->with('kdal', $kdal)->with('kokc', $kokc);
    }
    ...
    public function sendEmail()
    {
        ...
        #{
        #    $message->to($User->email)->subject('Welcome to vZFW');
        #});
    }
